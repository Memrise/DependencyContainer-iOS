...
class Test0AService : DependencyContainerTestsTestServiceProtocol {
}
...
class DependencyContainerTests: XCTestCase {
    ...
    func test_resolve_basic() {
        ...
        let s = DefaultDependencyAssembler(type: Test0BService.self){ (manager) -> Test0BService in
    ...
    func test_resolve_byProtocol() {
        ...
        let s = DefaultDependencyAssembler(type: DependencyContainerTestsTestServiceProtocol.self){ (manager) -> Test0BService in
            return Test0BService()
        }
        container.register(s)
        ...
        let res1 = container.resolve(by: DependencyContainerTestsTestServiceProtocol.self)
        XCTAssertNotNil(res1)
    ...
    func test_resolve_shortest_form() {
        ...
        let s = DefaultDependencyAssembler(type: Test0AService.self){ (manager) -> Test0AService in
            return Test0AService()
        }
        container.register(s)
    ...
    func test_assembler_resolve_dependencies() {
        ...
        let s1 = DefaultDependencyAssembler(type: Test0AService.self){ (manager) -> Test0AService in
            return Test0AService()
        }
